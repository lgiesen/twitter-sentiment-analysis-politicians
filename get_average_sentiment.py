...
def load_mean_count(json_filename, return_var=None):
    try:
        ...
        if not return_var:
            return data
        elif return_var == 'mean':
            return data['overall_mean']
        else:
...
if __name__ == '__main__':
    ...
    means.at['All','Total'] = load_mean_count('mean_count', 'mean')
    for city in cities:
        means.at['All',city] = load_mean_count(f'{city}_mean_count', 'mean')
    for country in countries:
    ...
    filepath = f'{data_path}mean_compound'
